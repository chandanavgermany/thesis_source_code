...
cpdef double[::1] compute_machine_makespans(int[:, ::1] operation_2d_array,
    ...
    for row in range(operation_2d_array.shape[0]):
        ...
        # machine_makespan_memory[machine] += runtime + wait + setup # for sequence dependent matrix
        machine_makespan_memory[machine] += runtime + wait + machine_setup_time_matrix[machine] # for machine setup time before processing operation
