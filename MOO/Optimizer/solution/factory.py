...
class SolutionFactory:
    ...
    def __init__(self, data, reschedule=False, preschedule_idle=False):
        """    
        ...
        reschedule : reschdule flag
        preschedule_idle : preschedule_idle flag for robust pro-active schedule
    ...
    def _generate_solution(self):
        ...
        if self.preschedule_idle:
            preschedule_idle = 1
        else:
            preschedule_idle = 0
        return Solution(self.jssp_instance_data, np.array(operation_list, dtype=np.intc),
                        reschedule=self.reschedule, preschedule_idle=preschedule_idle)
    ...
    def _generate_solution_w_processing_time_criteria(self, lpt):
        ...
        return Solution(self.jssp_instance_data, np.array(operation_list, dtype=np.intc), reschedule=self.reschedule,
