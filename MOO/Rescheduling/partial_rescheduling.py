# -*- coding: utf-8 -*-
"""
Created on Mon Mar 15 10:10:53 2021
...
class PartialRescheduling:
    def __init__(self, schedule, disruption_event_type, broken_machine=None, args=None):
    ...
    def reschedule(self, stability=0.4):
        objective_params = {'makespan': 0.6, 'stock_cost': 0, 'tardiness_cost': 0, 'stability': stability}
        ...
        unprocessed_job_operation_df.drop(['setup_start', 'setup_end','runtime_start', 'runtime_end',
                                           'machine_name','runtime_duration'], axis=1, inplace=True)
        if self.args.alg == 'hybrid':
            new_schedule, best_solution = main(machine_df, unprocessed_job_operation_df, objective_params, reschedule=True)
        else:
            new_schedule, best_solution = nsga(machine_df, unprocessed_job_operation_df, objective_params, reschedule=True)
            
        unprocessed_job_operation_df = unprocessed_job_operation_df.drop(['machine'], axis=1)
